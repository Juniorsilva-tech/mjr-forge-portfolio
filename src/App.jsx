...
const projects = [
  { slug: 'princessmel', label: 'Real Case', title: 'Princessmel', text: 'Landing page e identidade digital para loja de moda cristã, com foco em curadoria, apresentação de produtos, WhatsApp e atmosfera editorial.', tags: ['Landing Page', 'Branding', 'UI Editorial', 'WhatsApp'] },
  { slug: 'retailflow', label: 'SaaS Demo', title: 'RetailFlow Dashboard', text: 'Demo funcional de dashboard para pequenos negócios, com clientes, pedidos, pagamentos, relatórios e persistência local.', tags: ['React', 'Dashboard', 'CRUD', 'Vercel'], demo: 'https://retailflow-dashboard.vercel.app', repo: 'https://github.com/Juniorsilva-tech/retailflow-dashboard', screenshots: ['/retailflow/overview.png', '/retailflow/clientes.png', '/retailflow/pedidos.png', '/retailflow/financeiro.png', '/retailflow/relatorios.png'] },
  { slug: 'jarvis', label: 'Private System', title: 'Jarvis Workflow', text: 'Sistema experimental privado para organizar, revisar e acelerar criação de interfaces e aplicações web com apoio de IA.', tags: ['Automation', 'QA Visual', 'AI Workflow', 'React UI'] },
]

const sceneVariants = {
  depth: { hidden: { opacity: 0, y: 54, scale: 0.94 }, visible: { opacity: 1, y: 0, scale: 1 } },
  rise: { hidden: { opacity: 0, y: 72 }, visible: { opacity: 1, y: 0 } },
  left: { hidden: { opacity: 0, x: 90, scale: 0.98 }, visible: { opacity: 1, x: 0, scale: 1 } },
  expand: { hidden: { opacity: 0, scale: 0.9 }, visible: { opacity: 1, scale: 1 } },
  final: { hidden: { opacity: 0, y: 42, scale: 0.95 }, visible: { opacity: 1, y: 0, scale: 1 } },
}

function Scene({ type = 'rise', children, className = '', delay = 0 }) {
  return (
    <motion.div variants={sceneVariants[type] || sceneVariants.rise} initial="hidden" whileInView="visible" viewport={{ once: false, amount: 0.22, margin: '-80px' }} transition={{ duration: 0.9, delay, ease: [0.16, 1, 0.3, 1] }} className={className}>
      {children}
    </motion.div>
  )
}
  ...
  return (
    <div className="relative z-10 mx-auto max-w-[1500px] px-5 lg:px-8" aria-hidden="true">
      <div className="relative h-20 overflow-hidden">
        ...
        <motion.div initial={{ width: 0, opacity: 0 }} whileInView={{ width: '34%', opacity: 1 }} viewport={{ once: false, amount: 0.6 }} transition={{ duration: 1.1, ease: [0.16, 1, 0.3, 1] }} className={`absolute top-1/2 h-px bg-[#c7a15a]/80 ${align === 'right' ? 'right-0' : 'left-0'}`} />
        <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8 }} className={`absolute top-1/2 -translate-y-1/2 rounded-full border border-[#f4efe7]/10 bg-[#050505]/70 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.28em] text-[#c7a15a] backdrop-blur-xl ${align === 'right' ? 'right-0' : 'left-0'}`}>
          {label}
        </motion.div>
      </div>
    </div>
...
function Hero() {
  return <section id="top" className="relative z-10 min-h-screen overflow-hidden px-5 pb-16 pt-28 lg:px-8"><div className="forge-depth-stage relative mx-auto grid min-h-[calc(100vh-7rem)] max-w-[1500px] items-center gap-12 lg:grid-cols-[0.95fr_1.05fr]"><Scene type="depth"><div className="relative z-10 max-w-[880px]"><p className="mb-7 text-xs font-semibold uppercase tracking-[0.45em] text-[#c7a15a]">MJR Forge — Cinematic Front-end Experience</p><h1 className="text-[14vw] font-semibold leading-[.82] tracking-[-.10em] text-[#f4efe7] sm:text-[6.7rem] lg:text-[8.4rem]">Interfaces with presence.</h1><p className="mt-8 max-w-2xl text-lg leading-9 text-[#d8d0c3] md:text-xl">Sou Maurício Júnior. Desenvolvedor Front-end React em formação, criando landing pages, dashboards e experiências web com estética, produto, narrativa visual e entrega real.</p><div className="mt-10 flex flex-col gap-4 sm:flex-row"><Button href="#work">Explorar projetos</Button><Button href={BRAND.whatsapp} secondary>Falar comigo</Button></div></div></Scene><Scene type="left" delay={0.12}><div className="relative ml-auto w-full max-w-[560px]"><div className="absolute -inset-8 rounded-[3rem] bg-[#c7a15a]/10 blur-3xl" /><div className="relative overflow-hidden rounded-[3rem] border border-[#f4efe7]/10 bg-[#0b0a08]/70 p-3 shadow-[0_50px_160px_rgba(0,0,0,.55)]"><img src="/forge-portrait.jpg" alt="Maurício Júnior" className="h-[620px] w-full rounded-[2.4rem] object-cover object-[50%_42%] saturate-[.72] contrast-[1.12] brightness-[.72] sepia-[.12]" /><div className="absolute inset-3 rounded-[2.4rem] bg-[linear-gradient(180deg,transparent_35%,rgba(5,5,5,.92)_100%)]" /><div className="absolute bottom-8 left-8 right-8 rounded-3xl border border-[#f4efe7]/10 bg-[#050505]/55 p-5 backdrop-blur-xl"><p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#c7a15a]">Human Presence</p><p className="mt-2 text-sm leading-6 text-[#d8d0c3]">Foto real como linguagem visual: presença, textura e direção.</p></div></div></div></Scene></div></section>
}

function SectionTitle({ eyebrow, title, text, type = 'rise' }) {
  return <Scene type={type} className="mb-14 grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end"><div><p className="mb-4 text-xs font-semibold uppercase tracking-[0.35em] text-[#c7a15a]">{eyebrow}</p><h2 className="max-w-4xl text-4xl font-semibold leading-[0.98] tracking-[-0.055em] text-[#f4efe7] md:text-6xl">{title}</h2></div>{text && <p className="max-w-2xl text-lg leading-8 text-[#a89f91]">{text}</p>}</Scene>
}

function Manifesto() {
  return <section id="manifesto" className="relative z-10 mx-auto max-w-[1500px] px-5 py-16 lg:px-8"><Scene type="depth"><div className="relative overflow-hidden rounded-[3rem] border border-[#f4efe7]/10 bg-[#0b0a08]/75 p-8 shadow-[0_40px_140px_rgba(0,0,0,.45)] backdrop-blur-2xl md:p-14 lg:p-20"><div className="grid gap-14 lg:grid-cols-[.85fr_1.15fr] lg:items-end"><div><p className="mb-6 text-xs font-semibold uppercase tracking-[0.35em] text-[#c7a15a]">Manifesto</p><h2 className="text-5xl font-semibold leading-[.95] tracking-[-0.07em] text-[#f4efe7] md:text-7xl">O site não deve gritar. Deve conduzir.</h2></div><div><p className="text-xl leading-9 text-[#a89f91]">O Forge nasce da ideia de que uma boa interface tem ritmo: silêncio, tensão, respiro, impacto e clareza. Menos efeito genérico. Mais direção criativa.</p><div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-5">{principles.map((item, index) => <Scene key={item} type="rise" delay={index * 0.04}><div className="rounded-2xl border border-[#f4efe7]/10 bg-[#050505]/60 px-4 py-5 text-center text-sm font-semibold text-[#d8d0c3]">{item}</div></Scene>)}</div></div></div></div></Scene></section>
}

function JourneyScene() {
  return <section id="journey" className="relative z-10 mx-auto max-w-[1500px] px-5 py-16 lg:px-8"><SectionTitle type="left" eyebrow="Journey / rhythm" title="A jornada atravessa a tela." text="A timeline flui como galeria horizontal limpa dentro do scroll natural." /><div className="overflow-x-auto pb-4"><div className="flex min-w-max gap-5 pr-5">{milestones.map(([number, time, text], index) => <Scene key={time} type="left" delay={index * 0.07}><article data-cursor="active" className="forge-emerge-card relative h-[420px] w-[360px] rounded-[2.4rem] border border-[#f4efe7]/10 bg-[#0c0b09]/80 p-8 shadow-[0_40px_140px_rgba(0,0,0,.42)] backdrop-blur-2xl"><p className="text-[6rem] font-semibold leading-none tracking-[-.12em] text-[#c7a15a]/18">{number}</p><p className="mt-20 text-xs font-semibold uppercase tracking-[.32em] text-[#c7a15a]">{time}</p><p className="mt-5 text-base leading-8 text-[#a89f91]">{text}</p></article></Scene>)}</div></div></section>
}

function Work() {
  return <section id="work" className="relative z-10 mx-auto max-w-[1500px] px-5 py-16 lg:px-8"><SectionTitle type="expand" eyebrow="Selected Work" title="Projetos como peças editoriais." text="Cada projeto precisa explicar contexto, intenção e resultado — não apenas mostrar uma tela bonita." /><div className="grid gap-6">{projects.map((project, index) => <Scene key={project.title} type={index % 2 === 0 ? 'depth' : 'left'} delay={index * 0.08}><motion.article whileHover={{ y: -6 }} data-cursor="active" className="forge-emerge-card overflow-hidden rounded-[2.8rem] border border-[#f4efe7]/10 bg-[#0b0a08]/70 p-4 shadow-[0_40px_140px_rgba(0,0,0,.45)] backdrop-blur-2xl"><div className="grid gap-8 rounded-[2.4rem] bg-gradient-to-br from-[#f4efe7]/[0.035] via-transparent to-[#c7a15a]/[0.035] p-7 md:p-10 lg:grid-cols-[0.7fr_0.95fr_0.7fr] lg:items-center"><div><p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#c7a15a]">{project.label}</p><h3 className="mt-4 text-5xl font-semibold tracking-[-0.06em] text-[#f4efe7] md:text-6xl">{project.title}</h3></div><div><p className="text-lg leading-8 text-[#a89f91]">{project.text}</p><div className="mt-6 flex flex-wrap gap-2">{project.tags.map(tag => <span key={tag} className="rounded-full border border-[#f4efe7]/10 bg-[#f4efe7]/5 px-3 py-1 text-xs font-semibold text-[#d8d0c3]">{tag}</span>)}</div>{(project.demo || project.repo) && <div className="mt-7 flex flex-wrap gap-3">{project.demo && <Button href={project.demo}>Demo</Button>}{project.repo && <Button href={project.repo} secondary>GitHub</Button>}</div>}</div><ProjectMockup index={index} project={project} /></div></motion.article></Scene>)}</div></section>
}

function Process() {
  return <section id="processo" className="relative z-10 mx-auto max-w-[1500px] px-5 py-16 lg:px-8"><Scene type="rise"><div className="relative overflow-hidden rounded-[3rem] border border-[#f4efe7]/10 bg-[#0b0a08]/65 p-8 backdrop-blur-2xl md:p-14 lg:p-20"><p className="mb-6 text-xs font-semibold uppercase tracking-[0.35em] text-[#c7a15a]">Processo</p><h2 className="max-w-5xl text-5xl font-semibold leading-[.95] tracking-[-.07em] text-[#f4efe7] md:text-7xl">Do briefing ao refinamento.</h2><div className="mt-16 grid gap-4 md:grid-cols-5">{['Briefing', 'Direção', 'Build', 'Refino', 'Deploy'].map((item, index) => <Scene key={item} type="rise" delay={index * 0.06}><div className="forge-emerge-card rounded-[2rem] border border-[#f4efe7]/10 bg-[#050505]/50 p-6"><p className="mb-10 text-sm font-semibold text-[#c7a15a]">0{index + 1}</p><h3 className="text-2xl font-semibold tracking-[-0.04em] text-[#f4efe7]">{item}</h3></div></Scene>)}</div></div></Scene></section>
}

function Contact() {
  return <section id="contato" className="relative z-10 mx-auto max-w-6xl px-5 py-16 text-center lg:px-8"><Scene type="final"><div className="relative overflow-hidden rounded-[3rem] border border-[#f4efe7]/10 bg-[#0b0a08]/70 p-8 backdrop-blur-2xl md:p-16"><div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#c7a15a] to-transparent" /><p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#c7a15a]">Final Scene</p><h2 className="mx-auto mt-6 max-w-4xl text-4xl font-semibold tracking-[-0.06em] text-[#f4efe7] md:text-6xl">Vamos construir algo memorável.</h2><p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-[#a89f91]">Aberto para estágio Front-end React, freelas de landing pages, dashboards e experiências digitais premium.</p><div className="mt-10 flex flex-col justify-center gap-4 sm:flex-row"><Button href={BRAND.whatsapp}>WhatsApp</Button><Button href={`mailto:${BRAND.email}`} secondary>E-mail</Button><Button href={BRAND.github} secondary>GitHub</Button></div></div></Scene></section>
}
